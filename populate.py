...
def parsearxiv(arxivid):
	...
	req = urllib.request.Request(url, headers=generateheader())
	...
	for ident in sorted(literature):
		if not "doi" in literature[ident] and not "arxiv" in literature[ident] and not "usenix" in literature[ident]:
			if ident in biblio and "title" in biblio[ident]:
				print("## work {} has no DOI/arXiv/USENIX but a manual entry".format(ident))
			else:
		...
		if not ident in biblio or forced:
			...
			if "doi" in literature[ident]:
				...
				ft, fy, fa, fb = parseusenix(usenixurl)
...
print("- {}% USENIX entries".format(percent_usenix))
print("- {}% manual/other entries".format(100 - percent_arxiv - percent_doi - percent_usenix))
